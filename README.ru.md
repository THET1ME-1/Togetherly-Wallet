...
Ставите тег — дальше работает GitHub Actions:

```bash
git tag v0.6.1 && git push --tags
```

Прогон гоняет тесты, собирает по APK на каждую архитектуру и прикладывает их к
релизу. Положите в секреты репозитория четыре значения (`KEYSTORE_BASE64`,
`KEYSTORE_PASSWORD`, `KEY_ALIAS`, `KEY_PASSWORD`) — APK выйдут подписанными; без
них в имени появится `-unsigned`, потому что сборку на отладочном ключе нельзя
поставить поверх настоящей.

Для iOS есть отдельный прогон: он собирает IPA и отправляет его в TestFlight.
Ему нужны сертификат Apple, профиль и ключ App Store Connect в секретах —
имена перечислены в начале `.github/workflows/testflight.yml`.

...
class Store extends ChangeNotifier {
  ...
  /// Что делать с записями на устройстве после входа.
  ///
  /// Человек мог месяц писать без аккаунта. Молча уносить его записи в облако
  /// нельзя, а молча не уносить — он решит, что всё потерял. Поэтому
  /// спрашиваем один раз, и до ответа круг идёт без них.
  ///
  /// Только для хранилища, которое ещё ни разу не приходило с сервера. До
  /// 19.09.2026 синхронизированное хранилище на КАЖДОМ запуске выгружалось
  /// целиком: сервер звал партнёра «Новая запись» на простой вход, а старая
  /// копия своей операции ложилась поверх категории, которую партнёр успел
  /// поправить. Свои правки и без того ждут в очереди, она переживает
  /// перезапуск.
  void adoptLocalRecords() {
    if (_syncMark != 0) return;
    forgetSpareCash();
    if (_db.transactions.isNotEmpty) {
      pendingLocal = _db.transactions.length;
      notifyListeners();
    } else {
      markAllForSync();
    }
